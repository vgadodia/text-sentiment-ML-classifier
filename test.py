...
def check_perceptron_single_update():
    ...
    if check_tuple(
            ex_name, alg.perceptron_single_step_update,
            exp_res, feature_vector, label, theta, theta_0):
        return
    ...
    if check_tuple(
            ex_name + " (boundary case)", alg.perceptron_single_step_update,
            exp_res, feature_vector, label, theta, theta_0):
        return
...
def check_average_perceptron():
    ...
    exp_res = (np.array([1, 2]), 1)
    if check_tuple(
            ex_name, alg.average_perceptron,
            exp_res, feature_matrix, labels, T):
        return
...
def check_pegasos_single_update():
    ...
    if check_tuple(
            ex_name, alg.pegasos_single_step_update,
            exp_res,
    ...
    if check_tuple(
            ex_name +  " (boundary case)", alg.pegasos_single_step_update,
            exp_res,
            feature_vector, label, L, eta, theta, theta_0):
        return
...
def check_classify():
    ...
    exp_res = np.array([-1])
    if check_array(
            ex_name + " (boundary case)", alg.classify,
            exp_res, feature_matrix, theta, theta_0):
        return
...
def check_classifier_accuracy():
    ...
    T=1
    if check_tuple(
            ex_name, alg.classifier_accuracy,
            exp_res,
            alg.perceptron,
...
def check_extract_bow_feature_vectors():
    ...
    try:
        res = alg.extract_bow_feature_vectors(texts, dictionary)
    except NotImplementedError:
        log(red("FAIL"), ex_name, ": not implemented")
        return
